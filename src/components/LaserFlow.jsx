...
float vWisps(vec2 uv,float topF){
  ...
  return uWIntensity*sum*topF*span;
}
...
const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uFlowTime: { value: 0 },
      uFogTime: { value: 0 },
      uBeamXFrac: { value: horizontalBeamOffset ?? 0.8 },
      uBeamYFrac: { value: verticalBeamOffset ?? 0.5 },
    ...
    const animate = () => {
      ...
      if (!hasFadedRef.current) {
        const fadeDur = 0.6;
        const next = Math.min(1, uniforms.uFade.value + cdt / fadeDur);
        uniforms.uFade.value = next;
        if (next >= 1) hasFadedRef.current = true;
      }

      const tau = Math.max(1e-3, mouseSmoothTime);
      const alpha = 1 - Math.exp(-cdt / tau);
      const mx = mouseSmooth.x + (mouseTarget.x - mouseSmooth.x) * alpha;
      const my = mouseSmooth.y + (mouseTarget.y - mouseSmooth.y) * alpha;
      mouseSmooth.set(mx, my);
  ...
  useEffect(() => {
    ...
    if (typeof horizontalBeamOffset === 'number') uniforms.uBeamXFrac.value = horizontalBeamOffset;
    if (typeof verticalBeamOffset === 'number') uniforms.uBeamYFrac.value = verticalBeamOffset;
  ...
  // If a fromRef is provided, compute beam offsets to originate from that element's center
  useEffect(() => {
    if (!fromRef?.current || !mountRef.current || !uniformsRef.current) return;

    const updateOffsets = () => {
      const target = fromRef.current.querySelector?.('.big') || fromRef.current;
      const tr = target.getBoundingClientRect();
      const mr = mountRef.current.getBoundingClientRect();
      const cx = tr.left + tr.width / 2;
      const cy = tr.top + tr.height / 2;
      const xFrac = (cx - mr.left) / Math.max(1, mr.width);
      const yFrac = (cy - mr.top) / Math.max(1, mr.height);
      uniformsRef.current.uBeamXFrac.value = xFrac;
      uniformsRef.current.uBeamYFrac.value = yFrac;
    };

    updateOffsets();
    const onScroll = () => requestAnimationFrame(updateOffsets);
